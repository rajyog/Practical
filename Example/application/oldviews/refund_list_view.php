                    ...
                    <script type="text/javascript">
                        ...
                        function list_refund()
                        {
                            ...
                            $.ajax({
                                ...
                                url: '<?php echo base_url(); ?>refund/refund_list/',
                                success: function (data)
                                {
                                    var json_obj = $.parseJSON(data);
                                    var result_length = json_obj.refund_list.length;
                                    ...
                                    if (result_length > 0)
                                    {
                                        ...
                                        for (i = 0; i < json_obj.refund_list.length; i++)
                                        {
                                            output += '<tr>';
                                            output += "<td style='width:200px;'>" + json_obj.refund_list[i].account_holder_name + "</td>";
                                            output += "<td style='width:200px;'>" + json_obj.refund_list[i].account_no + "</td>";
                                            output += "<td style='width:200px;'>" + parseFloat(Math.round(json_obj.refund_list[i].amount * 100) / 100).toFixed(2);
                                            +"</td>";

                                            if (json_obj.refund_list[i].status == 'confirm')
                                            {
                                                output += "<td>Processed</td>";
                                            } else
                                            {
                                                output += "<td>" + json_obj.refund_list[i].status + "</td>";
                                            }


                                            output += "<td style='width:100px;'>";
                                            output += "<a href='<?php echo base_url(); ?>refund/view_refund/" + json_obj.refund_list[i].wr_id + "'><button  class='btn btn-success' title='View'><i class='fa fa-eye'></i></button></a>";
                                            output += "</td>";
                                            output += "</tr>";
                                        }
